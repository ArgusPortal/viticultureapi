...
class ProcessingScraper(BaseScraper):
    def get_processing_data(self, year=None):
        """
        Get combined processing data from all subcategories for a specific year or all years.
        ...
        try:
            ...
            if fallback_data and fallback_data.get("data") and len(fallback_data.get("data", [])) > 0:
                logger.info(f"Using CSV fallback for all processing data")
            ...
            subcategory_methods = [
                {'method': self.get_vinifera_processing, 'category': 'vinifera'},
                {'method': self.get_american_processing, 'category': 'american'},
                {'method': self.get_table_processing, 'category': 'table'},
                {'method': self.get_unclassified_processing, 'category': 'unclassified'}
            ]
            ...
            if combined_data:
                return {
                    "data": combined_data,
                    "source": "+".join(sources) if sources else "combined_processing",
                    "source_url": f"{self.BASE_URL}?opcao=opt_03"
                }
            
            # If we couldn't get any data, return empty result
            logger.warning("No processing data could be retrieved from any subcategory")
            return {"data": [], "source": "no_data_found", "source_url": f"{self.BASE_URL}?opcao=opt_03"}
        except Exception as e:
